...
- TaskAssignment: Tarefas atribuídas aos colaboradores
- AssignmentPhoto: Fotos das execuções
- Notification: Sistema de notificações
- push_token: Coluna para notificações push
"""
...
def run_migration():
    ...
    try:
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_task_assignments_due_date ON task_assignments(due_date);")
        ...
        # Verificar se a coluna is_read existe antes de criar o índice
        cursor.execute("""
        ...
        # 7. Habilitar extensão pgcrypto se não existir
        logger.info("Habilitando extensão pgcrypto...")
        cursor.execute("CREATE EXTENSION IF NOT EXISTS pgcrypto;")
        
        # 8. Criar função RPC para autenticação mobile
        logger.info("Criando função authenticate_user...")
        cursor.execute("""
            ...
            BEGIN
                RETURN QUERY
                SELECT u.id, u.username, u.full_name, u.team, u.role, u.active, u.created_at
                FROM users u
        ...
        if user_count == 0:
            ...
            """)
            logger.info("Usuários de exemplo criados")
